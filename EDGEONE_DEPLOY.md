...
3. **环境变量（运行时注入，必填）**：
   ...
   | 其他 | 邮件/图床 token 等按需 |
4. **Node Functions**：项目根的 `cloud-functions/[[default]].ts` 会被识别为 catch-all，自动挂载 `dist/index.mjs` 的 WinterTC handler（`export default function onRequest`）。EdgeOne Pages 要求 Node Functions 必须放在 `cloud-functions/` 目录，catch-all 入口文件名必须是 `[[default]].ts`。
...
| `cloud-functions/[[default]].ts` 路径/命名错误（旧用 `node-functions/[[...slug]].ts`） | 必须放在 `cloud-functions/` 目录，catch-all 入口文件名必须是 `[[default]].ts`；用 Node 脚本生成避免 Windows 路径拆分 |
