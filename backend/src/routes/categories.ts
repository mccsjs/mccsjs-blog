...
export function registerCategoryRoutes(app: any) {
  ...
  app.post('/api/categories', async ({ body, user, set }: any) => {
    if (!user) { set.status = 401; return { error: 'Unauthorized' }; }
    const data = categorySchema.parse(body);
    return prisma.category.create({ data });
  }, { auth: true });

  app.patch('/api/categories/:id', async ({ params, body, user, set }: any) => {
    if (!user) { set.status = 401; return { error: 'Unauthorized' }; }
    const data = categorySchema.partial().parse(body);
    return prisma.category.update({ where: { id: params.id }, data });
  }, { auth: true });

  app.delete('/api/categories/:id', async ({ params, user, set }: any) => {
    if (!user) { set.status = 401; return { error: 'Unauthorized' }; }
